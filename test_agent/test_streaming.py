...
class StreamingTester:
    ...
    async def test_streaming_performance(
        self, query: str, thread_id: str, iterations: int = 3
    ) -> Dict[str, Any]:
        ...
        for i in range(iterations):
            ...
            try:
                async for chunk in self.agent.chat_streaming(
                    user_query=query, thread_id=f"{thread_id}_iter_{i+1}"
                ):
        ...
        if successful_iterations:
            avg_time = sum(d["total_time"] for d in successful_iterations) / len(
                successful_iterations
            )
            avg_chunks = sum(d["chunks_received"] for d in successful_iterations) / len(
                successful_iterations
            )
    ...
    async def test_streaming_chunk_analysis(
        ...
        try:
            async for chunk in self.agent.chat_streaming(
                ...
                print(
                    f"[{chunk_type}] {content[:50]}{'...' if len(content) > 50 else ''}"
                )
    ...
    async def test_concurrent_streaming(
        self, queries: List[str], thread_id: str
    ) -> Dict[str, Any]:
        ...
        async def stream_single_query(query: str, query_id: int):
            ...
            try:
                async for chunk in self.agent.chat_streaming(
                    user_query=query, thread_id=f"{thread_id}_concurrent_{query_id}"
                ):
                    chunks.append(chunk)
    ...
    async def test_streaming_error_handling(self, thread_id: str) -> Dict[str, Any]:
        ...
        for i, query in enumerate(error_queries):
            ...
            try:
                async for chunk in self.agent.chat_streaming(
                    user_query=query, thread_id=f"{thread_id}_error_{i+1}"
                ):
                    chunks.append(chunk)
